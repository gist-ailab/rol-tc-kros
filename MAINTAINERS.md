...
> ⚠️ **확인 필요 — 폼이 로그인을 요구합니다.**
> 2026-08-31 기준으로 이 폼 주소는 로그인하지 않은 접속에 HTTP 401 을 돌려주며, 임베드 안에도
> "Google 계정에 로그인하십시오" 화면이 뜹니다. 폼 설정에서 응답자를 특정 조직으로 제한했거나
> 로그인을 요구하는 옵션이 켜져 있으면 외부 지원자가 신청하지 못합니다.
> 폼 편집 화면의 **설정 → 응답 → 이메일 주소 수집 / 응답자 제한** 항목을 확인하세요.
> 누구나 신청할 수 있게 하려면 조직 제한을 풀어야 합니다.
